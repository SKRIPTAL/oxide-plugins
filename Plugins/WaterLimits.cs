...
namespace Oxide.Plugins
{
    ...
    public class WaterLimits : CovalencePlugin
    {
        ...
        private void Init()
        {
            ...
            if (!config.HurtOnContact && !config.KillOnContact) Unsubscribe(nameof(OnRunPlayerMetabolism));
        ...
        private void WaterCheck(BasePlayer player)
        {
            ...
            if (config.HurtOnContact && player.metabolism.wetness.value == 0) return;
            if (config.HurtOnDisconnect && !IsInWater(player)) return;
        ...
        {
            foreach (var sleeper in BasePlayer.sleepingPlayerList) WaterCheck(sleeper);
        }
        ...
        private void OnPlayerDisconnected(BasePlayer player) => WaterCheck(player);

        private void OnRunPlayerMetabolism(PlayerMetabolism m, BaseCombatEntity entity)
        {
            var player = entity.ToPlayer();
            if (player != null) WaterCheck(player);
        }
