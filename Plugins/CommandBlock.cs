...
{
    [Info("Command Block", "Wulf/lukespragg", "0.3.0", ResourceId = 647)]
    [Description("Blocks configured commands sent by a player to the server")]
    public class CommandBlock : CovalencePlugin
    {
        ...
        private void BlockCommand(IPlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower())
            {
                ...
                case "list":
                    player.Reply("Blocked commands: " + string.Join(", ", config.BlockedCommands.ToArray())); // TODO: Localize
                    break;
        ...
        private object OnUserCommand(IPlayer player, string command, string[] args)
        {
            ...
            if (config.LogAttempts) LogWarning(Lang("CommandAttempted", player.Name, player.Id, command));
            return true;
        }
        ...
        private object OnServerCommand(ConsoleSystem.Arg arg)
        {
            ...
            if (config.LogAttempts) LogWarning(Lang("CommandAttempted", connection.username, connection.userid, arg.cmd.FullName));
            return true;
        }
#endif
        ...
        private object OnRunCommand(ConsoleSystem.Arg arg)
        {
            ...
            if (config.LogAttempts) LogWarning(Lang("CommandAttempted", netUser.displayName, netUser.userID, command));
            return true;
        }
#endif
