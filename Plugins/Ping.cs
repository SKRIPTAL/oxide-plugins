/*
 * TODO: Add optional text CUI for Rust only
 */
...
namespace Oxide.Plugins
{
    ...
    class Ping : CovalencePlugin
    {
        ...
        void PingKick(IPlayer player, string ping)
        {
            ...
            Puts(Lang("PlayerKicked", null, player.Name.Sanitize(), ping));
            server.Broadcast(Lang("PlayerKicked", null, player.Name.Sanitize(), ping));
        }
        ...
        void PingCommand(IPlayer player, string command, string[] args)
        {
            ...
            if (player.HasPermission(permCheck))
            {
                ...
                player.Reply(Lang("PlayerPing", player.Id, target.Name.Sanitize(), target.Ping));
            }
            else
