...
namespace Oxide.Plugins
{
    ...
    class CountryBlock : CovalencePlugin
    {
...
#if DEBUG
            PrintWarning($"Local: {IsLocalIp(ip)}, Admin: {(adminExcluded && player.IsAdmin)}, Perm: {player.HasPermission(permBypass)}");
#endif
            if (IsLocalIp(ip) || (adminExcluded && player.IsAdmin) || player.HasPermission(permBypass)) return;
...
#endif
                if ((!countryList.Contains(country) && whitelist) || (countryList.Contains(country) && !whitelist)) PlayerRejection(player, country);
            }, this);
        }
        ...
        {
            if (banInstantly) player.Ban(Lang("NotAllowed", player.Id, country));
