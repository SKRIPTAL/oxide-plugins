 ...
 * Add optional logging of advertisements/profanity
 ...
 * Fix filter bypassing by symbols around matches
 */
...
namespace Oxide.Plugins
{
    ...
    public class UFilter : CovalencePlugin
    {
        ...
        private List<object> allowedAds;
        private List<object> allowedProfanity;
        ...
        protected override void LoadDefaultConfig()
        {
            ...
            Config["Log Advertising (true/false)"] = logAdvertising = GetConfig("Log Advertising (true/false)", true);
            Config["Log Profanity (true/false)"] = logProfanity = GetConfig("Log Profanity (true/false)", true);
        ...
        private object HandleChat(IPlayer player, string message)
        {
            ...
            return string.IsNullOrEmpty(processed) ? (object)true : null;
        }
        ...
        private string ProcessText(string text, IPlayer player)
        {
            ...
            if (checkForAdvertising && advertisements.Length > 0)
            {
                ...
                {
                    if (logAdvertising)
                        foreach (var advertisement in advertisements) Log("advertising", $"{player.Name} ({player.Id}) {DateTime.Now}: {advertisement}");
            ...
            if (checkForProfanity && profanities.Length > 0)
            {
                ...
                {
                    if (logProfanity)
                        foreach (var profanity in profanities) Log("profanity", $"{player.Name} ({player.Id}) {DateTime.Now}: {profanity}");
        ...
        private void TakeAction(IPlayer player, string action, string reason)
        {
            switch (action)
            {
                case "ban":
        ...
        private void FilterCommand(IPlayer player, string command, string[] args)
        {
            ...
            switch (args[0]) // TODO: ToLower() handling
            {
                case "add":
        ...
        private object OnBetterChat(IPlayer player, string message) => true;

        private object OnUserChat(IPlayer player, string message) => HandleChat(player, message);

        private void OnUserRespawned(IPlayer player) // TODO: Unsubscribe
        {
        ...
        private void OnUserSpawned(IPlayer player) // TODO: Unsubscribe
        {
            if (!checkNames) return;
