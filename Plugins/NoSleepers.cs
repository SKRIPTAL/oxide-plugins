...
namespace Oxide.Plugins
{
    ...
    class NoSleepers : CovalencePlugin
    {
        ...
        void OnServerInitialized()
        {
            ...
            // Check if existing sleepers should be killed on load
            if (!killExisting) return;

            // Loop through sleepers list
            var killCount = 0;
            var sleepers = BasePlayer.sleepingPlayerList;
            foreach (var sleeper in sleepers.ToArray())
            {
                // Remove sleeping player from list
                if (sleeper.IsDead()) sleepers.Remove(sleeper);

                // Kill sleeping player
                sleeper.KillMessage();
                killCount++;
            }

            // Show amount of sleepers killed if any
