...
{
    [Info("Magic Description", "Wulf/lukespragg", "1.3.2", ResourceId = 1447)]
    ...
    public class MagicDescription : RustPlugin
    {
        ...
        protected override void LoadConfig()
        {
            ...
            try
            {
                ...
                if (config?.Description == null) LoadDefaultConfig();
            }
            catch
            {
        ...
        private void UpdateDescription(string text = "")
        {
            ...
            if (config.ShowPlugins)
            {
                ...
                foreach (var plugin in loadedPlugins.Where(p => !p.IsCorePlugin))
                {
        ...
        {
            if (!serverInitialized || !arg.IsAdmin) return null;
            if (arg.cmd.FullName != "server.description") return null;
